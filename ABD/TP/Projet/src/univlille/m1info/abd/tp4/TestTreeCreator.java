...
public class TestTreeCreator {

	private TreeQueryCreator tree;
	
	@Before
	public void setUp () {
		this.tree = new TreeQueryCreator();
	}
	...
	public static RAQuery getQuery_join_select_attrA_EQUAL_a1_from_REL_attrA_attrB_with_RELTWO_attrA_attrC () {
		...
		RAQuery join = new JoinQuery(relNameTwoQuery, leftSubQuery);
		return join;
	}
	
	public static RAQuery getQuery_select_join_proj(){
		RAQuery relNameQuery = new RelationNameQuery("RELONE");
		RAQuery selection = new SelectionQuery(relNameQuery, "attrA", ComparisonOperator.EQUAL, "a1");
		RAQuery relNameTwoQuery = new RelationNameQuery("RELTWO");
		RAQuery join = new JoinQuery(selection,relNameTwoQuery);
		String attributes[] = new String[]{"attrA","attrC"};
		RAQuery proj = new ProjectionQuery(join, attributes);
		return proj;
	}
	
	public static RAQuery getQuery_join_select_proj(){
		RAQuery relNameQuery = new RelationNameQuery("RELONE");
		RAQuery relNameTwoQuery = new RelationNameQuery("RELTWO");
		RAQuery join = new JoinQuery(relNameQuery,relNameTwoQuery);
		RAQuery selection = new SelectionQuery(join, "attrA", ComparisonOperator.EQUAL, "a1");

		String attributes[] = new String[]{"attrA","attrC"};
		RAQuery proj = new ProjectionQuery(selection, attributes);
		return proj;
	}
	
	@Test
	public void testSelectJoinProjection() {
		SimpleSGBD sgbd = new SimpleSGBD();

		// Create two relations
		SimpleDBRelation relation1 = sgbd.createRelation("RELONE", "attrA",
				"attrB");
		relation1.addTuple(new String[] { "a1", "b1" });
		relation1.addTuple(new String[] { "a1", "b3" });
		relation1.addTuple(new String[] { "a2", "b2" });

		SimpleDBRelation relation2 = sgbd.createRelation("RELTWO", "attrA",
				"attrC");
		relation2.addTuple(new String[] { "a1", "c1" });
		relation2.addTuple(new String[] { "a1", "c2" });

		RAQuery query = getQuery_select_join_proj();

		tree.setSGBD(sgbd);
		tree.createTreeQuery(query);
		// The operator that will compute the query
		PhysicalOperator operator = tree.getOperator();

		// Check that the operator returns the correct result
		List<String[]> expectedResult = new ArrayList<String[]>();
		expectedResult.add(new String[] { "a1","c1" });
		expectedResult.add(new String[] { "a1","c2" });
		expectedResult.add(new String[] { "a1","c1" });
		expectedResult.add(new String[] { "a1","c2" });
		
		List<String[]> obtainedResult = collectAllTuplesOfAnOperator(operator);
		if (!containTheSameTuples(expectedResult, obtainedResult)) {
			fail("Wrong result. Expected " + listToString(expectedResult)
					+ " Obtained " + listToString(obtainedResult));
		}
	}
	
	@Test
	public void testSelectJoinProjection_forme2() {
		SimpleSGBD sgbd = new SimpleSGBD();
		...
		RAQuery query = getQuery_join_select_proj();
	...
	@Test
	public void testJoinWithEpmtyResult() {
		SimpleSGBD sgbd = new SimpleSGBD();
