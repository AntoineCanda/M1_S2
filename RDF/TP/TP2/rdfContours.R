...
## Fonction qui annule des valeurs des descripteurs de fourier
rdfAnnuleDescFourrier<- function(desc, ratio){
  l <- length(desc)
  nbPointaEnlever <- l - l*ratio
  depart <- (l - nbPointaEnlever)/2 
  for(i in depart:(depart+nbPointaEnlever-1)){
    desc[i] <- 0
  }
...
distanceDroitePoint <- function(z, zA, zB){
    x <- Re(z)
    y <- Im(z)
    d <- equationDroite(Re(zA), Im(zA),Re(zB), Im(zB))
    abs(d[1]*x + d[2] * y + d[3]) / sqrt(d[1] ^ 2 + d[2] ^ 2)
}
...
rdfDistances <- function (cont) {
   ...
   ##   unlist(lapply(cont, function(z){distanceDroitePoint(z, debut, fin)}))
