...
<xsl:stylesheet version="2.0" 
	...
	<xsl:template match="/">
		<art xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">
			<artistes>
  			...
  			<oeuvres>
  				<xsl:apply-templates select="document('catalogues.xml')//fichier"></xsl:apply-templates>
  			</oeuvres>
  		</art>
	</xsl:template>
	...
	<xsl:template match="fichier">
		<xsl:variable name="nomFichier" select="./@uri"></xsl:variable>
		<xsl:apply-templates select="document($nomFichier)//oeuvre"></xsl:apply-templates>
	</xsl:template>
